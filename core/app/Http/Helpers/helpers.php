...
{
    return $image ?   getImage($image) : asset('assets/images/cover.png');
}
...
function userStars($rating)
{
    ...
    $halfStar = ($rating - $fullStars) >= 0.5 ? 1 : 0; // Check for half star
    ...
    for ($i = 0; $i < $fullStars; $i++) {
        $starsHtml .= '<i class="las la-star text-warning"></i>';
    }

    // Add half star if applicable
    if ($halfStar) {
        $starsHtml .= '<i class="las la-star-half-alt text-warning"></i>';
    }

    // Add empty stars
    for ($i = 0; $i < $emptyStars; $i++) {
        $starsHtml .= '<i class="lar la-star text-warning"></i>';
    }
...
function userLevel($user)
{
    ...
    if ($level) {
        ...
        notify($user, 'USER_LEVEL_UPGRADE', [
            ...
            'name'     => $level->name,
            'amount'   => showAmount($level->amount, currencyFormat: false),
        ]);
    }
}
...
function hasSlider()
{
    return request()->routeIs('home') ||
        ...
        request()->routeIs('software') ||
        request()->routeIs('software.details') ||
        request()->routeIs('job') ||
        request()->routeIs('job.details');
}
...
function getPermissions() {
    ...
    return Cache::rememberForever($permissionCacheKey, function () use ($admin) {

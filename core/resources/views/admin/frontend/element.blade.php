            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.frontend.sections.content', $key) }}" class="disableSubmission"
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <x-image-uploader class="w-100" :imagePath="frontendImage(
                                                    ...
                                                    id="image-upload-input{{ $loop->index }}" :size="isset($section->element->images->$imgKey->size)
                                                        ? $section->element->images->$imgKey->size
                                                        : null"
                                                    :required="false" />
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control iconPicker icon" autocomplete="off"
                                                name="{{ $k }}"
                                                value="{{ old($k, isset($data->data_values->$k) ? $data->data_values->$k : '') }}"
                                                required>
                                ...
                                @else
                                    @if ($content == 'textarea')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea rows="10" class="form-control" name="{{ $k }}" required>{{ old($k, isset($data->data_values->$k) ? $data->data_values->$k : '') }}</textarea>
                                            </div>
                                        </div>
                                    @elseif($content == 'textarea-nic')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>{{ __(keyToTitle($k)) }}</label>
                                                <textarea rows="10" class="form-control nicEdit" name="{{ $k }}">{{ old($k, isset($data->data_values->$k) ? $data->data_values->$k : '') }}</textarea>
                                            </div>
                                        </div>
                                    @elseif($k == 'select')
                                        @php
                                                ...
                                                <select class="form-control select2" data-minimum-results-for-search="-1"
                                                    ...
                                                    @foreach ($content->options as $selectItemKey => $selectOption)
                                                        <option value="{{ $selectItemKey }}"
                                                            @if ((isset($data->data_values->$selectName) ? $data->data_values->$selectName : '') == $selectItemKey) selected @endif>
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="slug"
                                                    value="{{ old($k, isset($data->slug) ? $data->slug : '') }}" required>
                                            </div>
                                        </div>
                                    @else
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <div class="d-flex justify-content-between">
                                                    <label>{{ __(keyToTitle($k)) }}</label>
                                                    @if ((isset($section->element->slug) ? $section->element->slug : '') == $k)
                        ...
                        <div class="form-group">
                            <button type="submit"
                                class="btn btn--primary @if (isset($section->element->slug) && !isset($data)) disabled @endif w-100 h-45">@lang('Submit')</button>
                        </div>
                    </form>
                </div>
            </div>
...
@push('script')
    <script>
        (function($) {
            ...
            @if (isset($section->element->slug) && $section->element->slug)
                ...
                $('[name=slug]').on('input', function() {
                    ...
                    if (slug) {
                        ...
                        `);
                        $.get("{{ route('admin.frontend.sections.element.slug.check', [$key, isset($data->id) ? $data->id : '']) }}", {
                            slug: slug
                        }, function(response) {
                            if (!response.exists) {
                                closestForm.find('.slug-verification').html(`

    ...
    <div
        ...
        <div style="width: 100% !important;">
            <div
                ...
                <div onclick="window.open('{{ route('public.profile', ['username' => optional($product->user)->username ?? ($product->username ?? 'user'), 'contact' => 'true']) }}', '_blank')"
                    ...
                    <div
                        ...
                        <span
                            ...
                            title="{{ $product->user ? $product->user->username : $product->username ?? 'user' }}">
                            {{ $product->user ? $product->user->username : $product->username ?? 'user' }}
                        </span>
                        <span
                ...
                @php
                    $cardAvgRating =
                        ($product->total_review ?? 0) > 0
                            ? number_format(($product->total_rating ?? 0) / $product->total_review, 1)
                            : '0.0';
                @endphp

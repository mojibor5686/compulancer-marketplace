@php
    $authUserId = auth()->id();
    if (@$productDetails) {
        $forLoadMoreReviewId = @$productDetails->id ?? @$user->id;
        $userReview = $productDetails->reviews()->where('user_id', $authUserId)->latest()->with('user')->first();
        $reviewPermission = $productDetails
            ->bookings()
            ->where('buyer_id', $authUserId)
            ->when($productDetails->software_id != 0, function ($query) {
                $query->where('status', Status::BOOKING_PAID);
            })
            ->when($productDetails->service_id != 0, function ($query) {
                $query->where('working_status', Status::WORKING_COMPLETED);
            })
            ->first();
    }

    $currentRouteName = Route::currentRouteName();
@endphp

<ul class="reviews-list reviews-list load-reviews">
    @if (@$userReview)
        @include('Template::partials.basic_review', ['review' => $userReview, 'authUser' => true])
    @endif


    @forelse($reviews->take(5) as $review)
        @php
            $isPublicProfile = $currentRouteName == 'public.profile';
            $showUserReview = $isPublicProfile ? true : $review->user_id != $authUserId;
        @endphp

        @if ($showUserReview)
            @include('Template::partials.basic_review')
        @endif
    @empty
        <div class="empty-message-box">
            <div class="icon"><svg xmlns="http://www.w3.org/2000/svg" version="1.1"
                    xmlns:xlink="http://www.w3.org/1999/xlink" x="0" y="0" viewBox="0 0 512 512"
                    style="enable-background:new 0 0 512 512" xml:space="preserve" class="">
                    <g>
                        <path fill-rule="evenodd"
                            d="M169.062 132.277H325.5a6.4 6.4 0 1 1 0 12.8H169.06a6.4 6.4 0 0 1-6.4-6.4c-.42 0 .693-6.4 6.4-6.4zm156.442 46.9a6.4 6.4 0 1 0 0-12.8H169.06a6.4 6.4 0 0 0 0 12.8zm-184.943 6.358a6.4 6.4 0 0 1-6.4 6.4h-59.63a6.4 6.4 0 0 1-6.4-6.4v-59.663a6.4 6.4 0 0 1 6.4-6.4h59.64a6.4 6.4 0 0 1 6.39 6.4v59.663zm-12.8-53.262h-46.84v46.862h46.861l-.022-46.862zm12.8 156.986a6.4 6.4 0 0 1-6.4 6.4h-59.63a6.4 6.4 0 0 1-6.4-6.4v-59.638a6.405 6.405 0 0 1 6.4-6.4h59.64a6.4 6.4 0 0 1 6.39 6.4v59.64zm-12.8-53.252h-46.84v46.862h46.861l-.022-46.864zm12.8 156.978a6.4 6.4 0 0 1-6.4 6.4H74.529a6.406 6.406 0 0 1-6.4-6.4v-59.628a6.406 6.406 0 0 1 6.4-6.4h59.64a6.4 6.4 0 0 1 6.39 6.4v59.632zm-12.8-53.242h-46.84v46.868h46.861l-.022-46.868zm353.482 135.335a18.577 18.577 0 0 0 .06-26.392l-44.573-44.978a14.648 14.648 0 0 0-15.523-3.4L398.272 377.3c.043-.453.067-.915.067-1.372a14.357 14.357 0 0 0-4.147-10.088l-.181-.188a99.613 99.613 0 0 0-15.339-120.04q-1.11-1.131-2.258-2.2V84.389a6.367 6.367 0 0 0-1.855-4.538l-63.444-62.117a6.177 6.177 0 0 0-4.4-1.734H25.226v451.186h351.25v-52.4l14.888 14.934a14.645 14.645 0 0 0 3.141 15.817l44.569 44.978a18.819 18.819 0 0 0 26.431.141l15.738-15.571zm-72.26-64.424-9.382 9.229-19.633-19.683 6.139-6.076 5.535-5.441 19.645 19.7-2.3 2.278zM307.934 228.875a86.926 86.926 0 0 1 72.758 134.174 6.4 6.4 0 0 0 .806 8l3.734 3.734a1.611 1.611 0 0 1 0 2.257l-8 7.905-8.892 8.747a1.6 1.6 0 0 1-2-.22l-3.734-3.734a6.4 6.4 0 0 0-8-.888 87.038 87.038 0 0 1-46.62 13.55c-39.806 0-67.577-23.285-82.257-58.824a87.585 87.585 0 0 1-4.588-27.94c0-14.575 2.905-23.528 8.132-36.8a86.914 86.914 0 0 1 78.661-49.96zm5.181-150.828-.015-40.465 41.338 40.465zM363.7 454.434V406.26a14.279 14.279 0 0 1-6.5-3.736l-.188-.187A99.561 99.561 0 0 1 238.1 386.629q-34.727 0-69.454.018a6.4 6.4 0 1 1 .413-12.787h58.034a98.419 98.419 0 0 1-11.733-21.332h-46.3a6.4 6.4 0 0 1 0-12.8h42.193a100.615 100.615 0 0 1 2.605-56.842l-44.8.006a6.4 6.4 0 0 1 0-12.8h50.243a99.8 99.8 0 0 1 14.833-21.34h-65.078a6.4 6.4 0 0 1 0-12.8h79.086a99.824 99.824 0 0 1 115.56-2.867V90.805h-56.99a6.4 6.4 0 0 1-6.4-6.4V28.8H38v425.634zm108.61 3.266a5.941 5.941 0 0 1-.02 8.323l-15.746 15.549a5.959 5.959 0 0 1-8.37-.041l-44.569-44.978a1.868 1.868 0 0 1 0-2.611l14.095-13.93 7.579-7.467a1.9 1.9 0 0 1 2.47.158zm-92.441-142.057a71.937 71.937 0 0 1-71.927 71.935c-39.433.2-71.547-32.782-71.547-71.931a71.737 71.737 0 1 1 143.474 0zm-71.923-59.161a59.432 59.432 0 0 1 59.32 59.173 59.241 59.241 0 1 1-59.32-59.173z"
                            fill="currentColor" opacity="1" data-original="currentColor" class=""></path>
                    </g>
                </svg></div>
            <p class="caption">@lang('No reviews available at this time.')</p>
        </div>
    @endforelse
</ul>


@if (@$userReview)
    <!-- Edit Review Modal -->
    <div class="modal fade" id="editReviewModal" tabindex="-1" aria-labelledby="editReviewModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editReviewModalLabel">@lang('Edit Review')</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="comment-form-area">
                        <form class="comment-form" action="{{ route('user.review.update', @$userReview->id) }}"
                            method="POST">
                            @csrf
                            <div
                                class="comment-ratings-area d-flex flex-wrap align-items-center justify-content-between">
                                <div class="rating">
                                    @for ($i = 5; $i >= 1; $i--)
                                        <input id="edit_star{{ $i }}" name="rating" type="radio"
                                            value="{{ $i }}"
                                            {{ $userReview && $userReview->rating == $i ? 'checked' : '' }} />
                                        <label for="edit_star{{ $i }}">&nbsp;</label>
                                    @endfor
                                </div>

                                <div class="like-dislike">
                                    <div class="d-flex flex-wrap align-items-center justify-content-sm-end">
                                        <div class="like-dislike me-4">
                                            <input id="edit-review-like" name="like" type="radio" value="1"
                                                {{ $userReview && $userReview->like_dislike == 1 ? 'checked' : '' }}>
                                            <label class="mb-0" for="edit-review-like"><i
                                                    class="fas fa-thumbs-up like-icon {{ $userReview && $userReview->like_dislike == 1 ? 'text-success' : '' }}"></i></label>
                                        </div>
                                        <div class="like-dislike">
                                            <input id="edit-review-dislike" name="like" type="radio" value="0"
                                                {{ $userReview && $userReview->like_dislike == 0 ? 'checked' : '' }}>
                                            <label class="mb-0" for="edit-review-dislike"><i
                                                    class="fas fa-thumbs-down dislike-icon {{ $userReview && $userReview->like_dislike === 0 ? 'text-danger' : '' }}"></i></label>
                                        </div>
                                    </div>
                                </div>
                            </div>

                            <textarea class="form-control form--control h-auto review-textarea" name="review" placeholder="@lang('Write Review')"
                                rows="8" required>{{ $userReview ? $userReview->review : '' }}</textarea>
                            <button class="submit-btn mt-20 mt-3 review-submit"
                                type="submit">@lang('Update')</button>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endif

<x-confirmation-modal />

@if (count($reviews) > 5)
    @php
        if (request()->routeIs('service.details')) {
            $type = 'service';
        } elseif (request()->routeIs('software.details')) {
            $type = 'software';
        } else {
            $type = 'profile';
        }
    @endphp

    <div class="text-center mt-4">
        <button class="btn btn--base loadMoreReviews" data-type="{{ $type }}">
            @lang('Load More')
        </button>
    </div>
@endif

@push('script')
    <script>
        (function($) {
            "use strict";

            var showReviews = 5;

            $('#edit-review-like').on('click', function() {
                $('.like-icon').addClass('text-success');
                $('.dislike-icon').removeClass('text-danger');
            });

            $('#edit-review-dislike').on('click', function() {
                $('.dislike-icon').addClass('text-danger');
                $('.like-icon').removeClass('text-success');
            });

            $('.loadMoreReviews').on('click', function(e) {
                e.preventDefault();
                var $btn = $(this);

                // Add loading spinner
                var originalHtml = $btn.html();
                $btn.html('<div class="spinner-border spinner-border-sm me-2"></div> @lang('Load more')...');

                $btn.addClass('btn-disabled').attr("disabled", true);

                var type = $btn.data('type');
                var skip = showReviews;

                $.ajax({
                    type: 'get',
                    url: '{{ route('fetch.reviews', $forLoadMoreReviewId) }}',
                    data: {
                        type: type,
                        skip: skip
                    },
                    dataType: "json",

                    success: function(response) {
                        if (response.success) {
                            $('.load-reviews').append(response.html);
                            showReviews += 5;

                            if (response.last) {
                                $btn.hide();
                            } else {
                                $btn.html(originalHtml).removeClass('btn-disabled').attr("disabled",
                                    false);
                            }
                        } else {
                            notify('error', response.error);
                            $btn.html(originalHtml).removeClass('btn-disabled').attr("disabled",
                                false);
                        }
                    },

                    error: function() {
                        notify('error', '@lang('Something went wrong. Please try again.')');
                        $btn.html(originalHtml).removeClass('btn-disabled').attr("disabled", false);
                    }
                });
            });
        })(jQuery);
    </script>
@endpush
